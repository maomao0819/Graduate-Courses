...
def main():
    ...
    for i in range(cfg.tta_num):
        for inputs, video_id, pid, start, end in tqdm.tqdm(test_loader):
            if inputs.numel() == 0:
                continue
    ...
    for inputs, video_id, pid, start, end in tqdm.tqdm(test_loader):
        if inputs.numel() == 0:
            ...
            count += 1
    print('In testing dataset, there are 2057 cases. 1486(72%) with bbox', 2057-outputs_all.shape[0], 'without bbox')
