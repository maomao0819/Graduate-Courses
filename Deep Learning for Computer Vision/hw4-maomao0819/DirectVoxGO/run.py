...
def config_parser():
    ...
    parser.add_argument("--data_transforms_path", type=str, default='hw4_data/hotdog',
                    help='data transforms path.')
    parser.add_argument("--save", type=str, default='',
                    help='the path to save the checkpoint.')
    parser.add_argument("--no_images", action='store_true')          
    parser.add_argument("--output_image_dir", type=str, default='images',
                    help='the directory to output images.')
    ...
    parser.add_argument("--render_exist", action='store_true', help='render exist data')
...
def load_everything(args, cfg):
    ...
    # args.data_transforms_path force to change
    # cfg.data['data_transforms_path'] default
    cfg.data['data_transforms_path'] = ''
    cfg.data['no_images'] = False
    if args.data_transforms_path:
        cfg.data['data_transforms_path'] = args.data_transforms_path
    if args.no_images:
        cfg.data['no_images'] = True
    data_dict, filenames_dict= load_data(cfg.data)
    ...
    return data_dict, filenames_dict
...
def scene_rep_reconstruction(args, cfg, cfg_model, cfg_train, xyz_min, xyz_max, data_dict, stage, coarse_ckpt_path=None):
    ...
    for global_step in trange(1+start, 1+cfg_train.N_iters):
        ...
        if global_step%args.i_weights==0:
            if args.save:
                ...
                path = os.path.join(args.save, f'{stage}_{global_step:06d}.tar')
            else:
    ...
    if args.save:
        os.makedirs(args.save, exist_ok=True)
        last_ckpt_path = os.path.join(args.save, f'{stage}_last.tar')
...
def train(args, cfg, data_dict):
    ...
    if cfg.coarse_train.N_iters > 0:
        ...
        if args.save:
            coarse_ckpt_path = os.path.join(args.save, f'coarse_last.tar')
        else:
...
if __name__=='__main__':
    ...
    # load images / poses / camera settings / data split
    data_dict, filenames_dict = load_everything(args=args, cfg=cfg)
    ...
    if args.render_test or args.render_val or args.render_train or args.render_video:
        if args.ft_path:
            ...
            ckpt_path = os.path.join(args.save, f'fine_last.tar')
        else:
    ...
    def render(split='train'):
        testsavedir = os.path.join(cfg.basedir, cfg.expname, f'render_{split}_{ckpt_name}')
        os.makedirs(testsavedir, exist_ok=True)
        print('All results are dumped into', testsavedir)
        rgbs, depths, bgmaps = render_viewpoints(
                render_poses=data_dict['poses'][data_dict[f'i_{split}']],
                HW=data_dict['HW'][data_dict[f'i_{split}']],
                Ks=data_dict['Ks'][data_dict[f'i_{split}']],
                gt_imgs=[data_dict['images'][i].cpu().numpy() for i in data_dict[f'i_{split}']],
                savedir=testsavedir, dump_images=args.dump_images,
                eval_ssim=args.eval_ssim, eval_lpips_alex=args.eval_lpips_alex, eval_lpips_vgg=args.eval_lpips_vgg,
                **render_viewpoints_kwargs)

        if args.render_image:
            filenames = filenames_dict[split]
            # images = np.concatenate((utils.to8b(rgbs), utils.to8b(1 - bgmaps * depths / np.max(depths))), axis=-1)
            images = utils.to8b(rgbs)
            print('Generating images')
            
            if './/' in args.output_image_dir or '/home' in args.output_image_dir:
                args.output_image_dir = args.output_image_dir[3:]
            for image_id in trange(len(images)):
                os.makedirs(os.path.join(args.output_image_dir), exist_ok=True)
                imageio.imwrite(os.path.join(args.output_image_dir, os.path.basename(filenames[image_id])), images[image_id])
        else:
