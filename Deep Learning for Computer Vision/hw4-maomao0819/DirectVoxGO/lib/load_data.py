...
def load_data(args):
    ...
    if args.dataset_type == 'llff':
        ...
        if args.data_transforms_path:
            images, poses, render_poses, hwf, i_split, filenames_dict = load_blender_data(args.no_images, args.data_transforms_path, args.half_res, args.testskip)
        else:
            images, poses, render_poses, hwf, i_split, filenames_dict = load_blender_data(args.no_images, args.datadir, args.half_res, args.testskip)
        
        print('Loaded blender', images.shape, render_poses.shape, hwf, args.datadir)
        # i_train, i_val, i_test = i_split
        i_train = i_split['train']
        i_val = i_split['val']
        i_test = i_split['test']

        near, far = 2., 6.

        if images.shape[-1] == 4 and not args.no_images:
            if args.white_bkgd:
