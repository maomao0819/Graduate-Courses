...
class UnwrappedFaceWeightedAverage(nn.Module):
    def __init__(self, output_num_channels=2, input_num_channels=3, inner_nc=512, skip_net_backbone='unet_3+', no_skip_net_backbone='unet_128'):
        super(UnwrappedFaceWeightedAverage, self).__init__()

        self.pix2pixUnwrapped = Pix2PixModel(3, which_model_netG=skip_net_backbone)

        self.pix2pixSampler = NoSkipPix2PixModel(input_num_channels, output_num_channels, inner_nc=inner_nc, no_skip_net_backbone=no_skip_net_backbone)
    ...
    def forward(self, target_pose, *input_imgs):
        ...
        for i in range(0, len(input_imgs)):
            ...
            input_imgs_t[i] = nn.functional.grid_sample(
                input_imgs[i], input_imgs_t[i], align_corners=True).unsqueeze(4)
        ...
        sampler_xy = sampler_xy.permute(0, 2, 3, 1) + stddev.permute(0, 2, 3, 1).contiguous().mul(Variable(torch.randn(
            xs.size()).cuda(), requires_grad=False)) + Variable(xs, requires_grad=False)  # choose values according to the std dev
    ...
    def get_unwrapped(self, *input_imgs):
        ...
        return result_xc, input_imgs, confidences_sum
    ...
    def get_result_w_embedding(self, embedding, target_pose):
        xs = np.linspace(-1, 1, embedding.shape[2])
        xs = np.meshgrid(xs, xs)
        xs = np.stack(xs, 2)

        xs = torch.Tensor(xs).unsqueeze(0).repeat(embedding.shape[0], 1, 1, 1).cuda()
        ...
        if sampler.size(1) == 2:
            ...
            sampled_image = nn.functional.grid_sample(embedding, sampler_xy, align_corners=True)
            return sampled_image
...
class UnwrappedFaceWeightedAveragePose(nn.Module):
    def __init__(self, output_num_channels=2, input_num_channels=3, inner_nc=512, input_pose=True):
        ...
        self.pix2pixSampler = NoSkipPix2PixModel_pose(
            input_num_channels, output_num_channels, input_pose=input_pose, inner_nc=inner_nc)
