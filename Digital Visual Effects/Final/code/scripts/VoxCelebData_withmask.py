...
class VoxCeleb(data.Dataset):
    def __init__(self, num_views, random_seed, dataset, dim=128):
        super(VoxCeleb, self).__init__()
        self.rng = np.random.RandomState(random_seed)

        # Update the npz files with the name that you downloaded it to from the website
        assert (os.path.exists('./files/landmarks_samevideoimg_%d25thframe_5imgs.npz' % (dataset)))
        files = np.load('./files/landmarks_samevideoimg_%d25thframe_5imgs.npz' % (dataset))
        # assert(os.path.exists('../files/landmarks_samevideoimg_%d25thframe_5imgs_%d.npz' % (dataset, num_views)))
        # files = np.load('../files/landmarks_samevideoimg_%d25thframe_5imgs_%d.npz' % (dataset, num_views))

        self.image_names = files['image_names']
        self.input_indices = files['input_indices']
        self.landmarks = files['landmarks']
        self.num_views = num_views
        self.transform = Compose([Resize((dim, dim)), ToTensor()])
    ...
    def get_blw_item(self, index):
        ...
        for i in range(0, self.num_views):
            ...
            img_name = img_name.replace('/scratch/local/ssd/koepke/voxceleb/faces', './faces')
