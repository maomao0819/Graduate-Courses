...
# randomly choose 3 points to find the fittest homography
...
def solve_p3p(p2D, p3D, check_V, checkX):
    ...
    for root_idx in range(len(a)):
        ...
        for translation_idx in range(translation.shape[0]):
            ...
            lambdas = (
                lambda_signs
                * distance(p3D.T - translation[translation_idx].reshape((3, 1)), axis=0)
                / distance(v.T, axis=0)
            )
            # try ech lambda
            for each_lambda in lambdas:
                # find proper R with trial and error
                ...
                if np.abs(det_rotation - 1) < 1e-3 and np.allclose(
                    np.dot(rotation.T, rotation), [[1, 0, 0], [0, 1, 0], [0, 0, 1]]
                ):
...
def pnpsolver(query, model, intrinsic_parameters, distortion_parameters):
    ...
    # find best translation and rotation with ranseec
...
def calculate_error(tList, tvec_gts, rotqs, rotq_gts):
    ...
    for idx in range(len(rotqs)):
        q = R.from_matrix(
            R.from_quat(rotqs[idx]).as_matrix() @ np.linalg.inv(R.from_quat(rotq_gts[idx]).as_matrix())
        ).as_quat()
        angle = qtoa(q[0])
...
def get_sorted_img_idx(images_df, set_name="train"):
    ...
    n_points = [
        int(float(re.findall(r"-?\d+\.?\d*", image_name_series[image_idx])[0]))
        for image_idx in range(len(image_name_series))
        if set_name in image_name_series[image_idx]
    ]
    images_index = np.argsort(n_points)
...
if __name__ == "__main__":
    ...
    if (os.path.exists("translation_pred.npy") or os.path.exists("rotation_pred.npy")) == False:
        ...
        for image_idx in images_pbar:
            ...
            rotq, tvec, n_inlier = pnpsolver(
                (kp_query, desc_query), (kp_model, desc_model), intrinsic_parameters, distortion_parameters
            )
    ...
    # already has translation and rotation
    else:
        images_df = pd.read_pickle("data/images.pkl")
        tvec_gts = []
        rotq_gts = []
