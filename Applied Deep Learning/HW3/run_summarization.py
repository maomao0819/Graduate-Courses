...
class DataTrainingArguments:
    ...
    pred_file: Optional[str] = field(
        ...
        metadata={
            "help": "An optional output prediction data file (a jsonlines or csv file)."
        },
    )
...
def main():
    ...
    pred_with_label = data_args.pred_with_label 
    ...
    if data_args.summary_column is None:
        ...
        if pred_with_label and summary_column not in column_names:
            raise ValueError(
    ...
    def preprocess_function_without_label(examples):
        # remove pairs where at least one record is None
        inputs = []
        for i in range(len(examples[text_column])):
            if examples[text_column][i] and examples[summary_column][i]:
    ...
    if training_args.do_predict:
        ...
        with training_args.main_process_first(desc="prediction dataset map pre-processing"):
            removed_column_names = column_names.copy()
            if 'id' in removed_column_names:
                removed_column_names.remove('id')
            preprocess_function_map = preprocess_function if pred_with_label and summary_column in column_names else preprocess_function_without_label
            if summary_column not in column_names:
                summary_column_content = ["null"] * len(predict_dataset)
                predict_dataset = predict_dataset.add_column(summary_column, summary_column_content)
            predict_dataset = predict_dataset.map(
                preprocess_function_map,
                batched=True,
                num_proc=data_args.preprocessing_num_workers,
                remove_columns=removed_column_names,
                load_from_cache_file=not data_args.overwrite_cache,
        ...
        if pred_with_label:
            ...
            if trainer.is_world_process_zero():
                if training_args.predict_with_generate:
                    ...
                    #     writer.write("\n".join(predictions))
                    utils.save_jsonl(predict_dataset['id'], predictions, data_args.pred_file)
        else:
            predict_dataloader = trainer.get_test_dataloader(predict_dataset)
            generations = utils.generate(trainer.model, predict_dataloader, trainer.tokenizer, torch.device('cuda'), do_sample, num_beams, temperature, top_k, top_p, max_length)
            generations = [gen.strip() for gen in generations]
            utils.save_jsonl(predict_dataset['id'], generations, data_args.pred_file)
